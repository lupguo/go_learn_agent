...
import (
	...
	"github.com/lupguo/go_learn_agent/internal/s06_compact"
...
func main() {
	...
	cwd, _ := os.Getwd()
	compactState := &s06_compact.CompactState{}
	...
	registry.Register(s06_compact.NewCompactTool())

	agent := s06_compact.New(provider, registry, compactState, cwd)

	var history []llm.Message
	scanner := bufio.NewScanner(os.Stdin)
	ctx := context.Background()

	fmt.Println("s06 Context Compact — type 'q' to quit")
	for {
		...
		// Update history from state (may have been compacted)
