...
import (
	...
	"github.com/lupguo/go_learn_agent/internal/s08_hooks"
...
func main() {
	...
	cwd, _ := os.Getwd()
	hooks := s08_hooks.NewHookManager(cwd, "")

	// Fire SessionStart hooks
	hooks.RunHooks(s08_hooks.EventSessionStart, &s08_hooks.HookContext{})
	...
	agent := s08_hooks.New(provider, registry, hooks)

	var history []llm.Message
	scanner := bufio.NewScanner(os.Stdin)
	ctx := context.Background()

	fmt.Println("s08 Hook System — type 'q' to quit")
	for {
