...
// Agent is the s15 lead agent that coordinates a team of teammates.
type Agent struct {
...
// New creates a new s15 lead agent.
func New(provider llm.Provider, registry *tool.Registry, bus *MessageBus) *Agent {
	...
	return &Agent{
		...
		registry:  registry,
		bus:       bus,
		system:    fmt.Sprintf("You are a team lead at %s. Spawn teammates and communicate via inboxes.", cwd),
		maxTokens: 8000,
	}
}
